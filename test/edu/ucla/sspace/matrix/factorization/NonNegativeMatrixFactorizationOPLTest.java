 ...
 */
public class NonNegativeMatrixFactorizationOPLTest {
    ...
    @Test public void testReduction() {
        MatrixFactorization reducer =
            new NonNegativeMatrixFactorizationOPL();
        SparseMatrix matrix = new YaleSparseMatrix(4,3);
        for (int r = 0; r < 4; r++)
            for (int c = 0; c < 3; c++)
                if (VALUES[r][c] != 0d)
                    matrix.set(r, c, VALUES[r][c]);
