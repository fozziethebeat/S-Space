...
val pos = if (args(0) == "all") "" else args(0)
val solutionSet = readKey(args(1), pos)
val numMappings = (args.size - 2) / 2
val allMappings = args.slice(2, 2+numMappings)
val allEvaluations = args.slice(2+numMappings, args.size)

var average = 0.0
for ( (mapFile,evalFile) <- allMappings zip allEvaluations) {
    val mappingSet = readKey(mapFile, pos)
    val evaluationSet = readKey(evalFile, pos)
    ...
    average += (correct/total.toDouble)
}
printf("%f\n", average / numMappings)

def readKey(keyFile: String, pos: String) =
    Source.fromFile(keyFile).getLines.map(_.split("\\s+")).filter(_(0).endsWith(pos)).toList.groupBy(_(0))
