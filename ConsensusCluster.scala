...
object ConsensusCluster {
     ...
     */
    def columnSubSample(matrix: Matrix, 
                        indcators: Matrix, 
                        fraction: Double): SparseMatrix = {
        matrix match {
            case sm:SparseMatrix => {
                val indices = 0 until matrix.columns
     ...
     *         matrix}, without replacement.
     */
    def rowSubSample(matrix: Matrix, indicators: Matrix, fraction: Double) = {
        matrix match {
            case sm:SparseMatrix => {
                ...
                val columnMap:Array[Int] = 0 until matrix.columns toArray
                val m = new CellMaskedSparseMatrix(sm, rowMap, columnMap)
    ...
    def findClusters(assignments: Assignments, rowMap: Array[Int]) = {
        ...
        for ((assignment, i) <- assignments.assignments zipWithIndex)
            clusters(assignment.assignments()(0)).add(
    ...
    def printConsensusMatrix(matrix: Matrix, indicators: Matrix,
        ...
        for (row <- 0 until matrix.rows) {
            for (col <- 0 until row)
                writer.print("%f ".format(
                    consensus(matrix, indicators, row, col)))
            writer.println
        }
        writer.close
    }
    ...
    def main(args:Array[String]) {
        if (args.length != 6) {
            println("usage: ConsensusCluster <ClusterAlg> <InputMatrix> " +
                    "<maxK> <numSamples> <samplePercent> <datFileBase>")
            return
        }
        ...
        println("Computing the reference clusters")
        (2 to maxK).par foreach {k =>
            printf("Running reference clustering k: %d\n", k)
            val assignments = clustering.cluster(dataset, k, props)
            val writer = new PrintWriter("%s.rca%02d".format(args(5), k))
            writer.println("%d %d".format(dataset.rows,assignments.numClusters))
        ...
        val results = 2 to maxK map { k =>
            ...
            val gapScores = (1 to numSamples).par map { h =>
                printf("Subsampling dataset k: %d, h: %d\n", k, h)
                val (sampledMatrix, rowMap) = rowSubSample(
                    dataset, indicators, dimFraction)
            ...
            printConsensusMatrix(adjacency, indicators, k, args(5))
            ...
            for (x <- 0 until adjacency.rows; y <- 0 until x)
                scoreCounter.count(adjacency.get(x, y)/indicators.get(x,y))
